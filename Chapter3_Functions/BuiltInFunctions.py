...
We’ll learn more about them later in the book.


From Import Statements
----------------------
An alternative form of the import statement is composed of the from keyword, followed by the module name, the import
...
When response is equal to exit, the program ends. Since the response variable is set by the input() function,
the user must enter exit in order to stop the program.

"""
pyper = """PyperClip - Install pyperclip at home using pip install pyperclip
It has a copy and paste function in it
...
while True:
    ...
    response = input()
    if response == 'exit':
        print('You typed ' + response + '.', 'exiting this program.')
        sys.exit()
    elif response != 'exit':
        print('You didnt type exit, exiting this program anyway...')
        sys.exit()
